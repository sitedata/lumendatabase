...
describe User, type: :model do
  ...
  context '#role?' do
    it 'returns true when the user has the given role' do
      ...
      expect(user).to be_role(Role.redactor)
      expect(user).to be_role(Role.publisher)
    end

    it 'returns false when the user does not have the given role' do
      user = create(:user, roles: [Role.redactor, Role.publisher])

      expect(user).not_to be_role(Role.admin)
      expect(user).not_to be_role(Role.super_admin)
    end
  end
end
